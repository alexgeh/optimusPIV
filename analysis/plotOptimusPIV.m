...
% rootDir = "R:\ENG_Breuer_Shared\agehrke\DATA\2025_optimusPIV\20250814_bayes_opt_2\"; % One parameter successful optimization
rootDir = "R:\ENG_Breuer_Shared\agehrke\DATA\2025_optimusPIV\20250815_bayes_opt\"; % Two parameter successful optimization
...
for mpti = 1:nMpt
% for mpti = [1,5,18,27]
    caseString = sprintf('ms%04d', mpti);
    ...
    nPhavg = 10;
    for phavgi = 1:nPhavg
        u_phavg(:,:,phavgi) = mean(u_crop(:,:,phavgi:nPhavg:end),3);
        o_phavg(:,:,phavgi) = mean(o_crop(:,:,phavgi),3);
    end
...
%     cLimits = [0.5 1.5];
% 
%     outFolder = fullfile(plotDir, caseString, "u_comp_filt");
...
% %         toplot = u_phavg(:,:,framei) / Uinf;
%                 toplot = smooth2g(toplot, 8, 4);
...
%     end
    ...
    outFolder = fullfile(plotDir, caseString, "vorticity_filt_lim");
    ensureTopLevelFolder(fullfile(plotDir, caseString));
    ensureTopLevelFolder(outFolder);
